...
const EditInvoicePage = () => {
  ...
  const handleOrderChange = (
    ...
    if (field === 'unitPrice' || field === 'units') {
      updatedOrders[index].unitTotalPrice =
        roundToTwo(updatedOrders[index].unitPrice * updatedOrders[index].units);
    }
  ...
  const calculateTotalCost = (orders: any[]) => {
    return roundToTwo(orders.reduce((total, order) => total + order.unitTotalPrice, 0));
  };

  const calculateVAT = (orders: any[]) => {
    return roundToTwo(orders.reduce((totalVAT, order) => totalVAT + order.units, 0));
  };

  const calculateGrandTotal = (totalCost: number, vat: number) => {
    return roundToTwo(totalCost + (totalCost * vat) / 100);
  };
    ...
    (<Box>
            ...
            <TableBody>
              {editedInvoice.orders.map((order: any, index: number) => (
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Typography variant="body1">{formatAmount(order.unitTotalPrice)}</Typography>
                  </TableCell>
                  <TableCell>
      ...
      <Box p={3} bgcolor="primary.light" mt={3}>
        <Box display="flex" justifyContent="end" gap={3} mb={3}>
          ...
          <Typography variant="body1" fontWeight={600}>
            {formatAmount(editedInvoice.totalCost)}
          </Typography>
        </Box>
        <Box display="flex" justifyContent="end" gap={3} mb={3}>
          <Typography variant="body1" fontWeight={600}>
          ...
          <Typography variant="body1" fontWeight={600}>
            {formatAmount(editedInvoice.vat)}
          </Typography>
        </Box>
        <Box display="flex" justifyContent="end" gap={3}>
          ...
          <Typography variant="body1" fontWeight={600}>
            {formatAmount(editedInvoice.grandTotal)}
          </Typography>
        </Box>
      </Box>
      {showAlert && (
